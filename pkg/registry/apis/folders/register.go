...
var resourceInfo = foldersv1.FolderResourceInfo
...
func (b *FolderAPIBuilder) UpdateAPIGroupInfo(apiGroupInfo *genericapiserver.APIGroupInfo, opts builder.APIGroupOptions) error {
	...
	selectableFieldsOpts := grafanaregistry.SelectableFieldsOptions{
		GetAttrs: fieldselectors.BuildGetAttrsFn(foldersv1.FolderKind()),
	}
	unified, err := grafanaregistry.NewRegistryStoreWithSelectableFields(opts.Scheme, foldersv1.FolderResourceInfo, opts.OptsGetter, selectableFieldsOpts)
	if err != nil {
		return err
	}
	...
	if b.folderSvc != nil {
		legacyStore := &legacyStorage{
			...
			tableConverter: foldersv1.FolderResourceInfo.TableConverter(),
		}
		dw, err := opts.DualWriteBuilder(foldersv1.FolderResourceInfo.GroupResource(), legacyStore, unified)
		if err != nil {
			return err
		}
	...
	storage[foldersv1.FolderResourceInfo.StoragePath()] = b.storage

	b.parents = newParentsGetter(b.storage, b.maxNestedFolderDepth) // used for validation
	storage[foldersv1.FolderResourceInfo.StoragePath("parents")] = &subParentsREST{
		getter:  b.storage,
		parents: b.parents,
	}
	storage[foldersv1.FolderResourceInfo.StoragePath("counts")] = &subCountREST{
		getter:   b.storage,
		searcher: b.searcher,
	}
	storage[foldersv1.FolderResourceInfo.StoragePath("access")] = &subAccessREST{
		getter:       b.storage,
		accessClient: b.accessClient,
	}

	// Adds a path to return children of a given folder
	storage[foldersv1.FolderResourceInfo.StoragePath("children")] = &subChildrenREST{
		getter: b.storage,
		lister: b.storage,
	}

	// Serve the same storage under both v1 and v1beta1 (conversion is handled by the scheme).
	apiGroupInfo.VersionedResourcesStorageMap[foldersv1.VERSION] = storage
	apiGroupInfo.VersionedResourcesStorageMap[foldersv1beta1.VERSION] = storage
	return nil
}
...
func (b *FolderAPIBuilder) GetOpenAPIDefinitions() common.GetOpenAPIDefinitions {
	return func(ref common.ReferenceCallback) map[string]common.OpenAPIDefinition {
		...
		// Register v1 types with the same OpenAPI schema as v1beta1 (identical structure).
		// The scheme references v1 type names, so they must be present for OpenAPI generation.
		v1Types := []struct{ v1, v1beta1 string }{
			{foldersv1.Folder{}.OpenAPIModelName(), foldersv1beta1.Folder{}.OpenAPIModelName()},
			{foldersv1.FolderList{}.OpenAPIModelName(), foldersv1beta1.FolderList{}.OpenAPIModelName()},
			{foldersv1.FolderSpec{}.OpenAPIModelName(), foldersv1beta1.FolderSpec{}.OpenAPIModelName()},
			{foldersv1.FolderInfo{}.OpenAPIModelName(), foldersv1beta1.FolderInfo{}.OpenAPIModelName()},
			{foldersv1.FolderInfoList{}.OpenAPIModelName(), foldersv1beta1.FolderInfoList{}.OpenAPIModelName()},
			{foldersv1.FolderAccessInfo{}.OpenAPIModelName(), foldersv1beta1.FolderAccessInfo{}.OpenAPIModelName()},
			{foldersv1.DescendantCounts{}.OpenAPIModelName(), foldersv1beta1.DescendantCounts{}.OpenAPIModelName()},
			{foldersv1.ResourceStats{}.OpenAPIModelName(), foldersv1beta1.ResourceStats{}.OpenAPIModelName()},
		}
		for _, t := range v1Types {
			if d, ok := defs[t.v1beta1]; ok {
				defs[t.v1] = d
			}
		}
		return defs
